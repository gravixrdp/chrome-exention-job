...
import { detectPlatform, isJobPage, extractIndeedJobData } from '../src/services/detector.js';
...
function init() {
  ...
  if (platform === 'Indeed' && isJobPage(url, platform)) {
    detectJob();
    observePageChanges();
  }
}
...
function detectJob() {
  currentJobData = extractIndeedJobData();
...
function observePageChanges() {
  ...
  const observer = new MutationObserver(() => {
    ...
    if (currentUrl !== lastUrl) {
      lastUrl = currentUrl;
      if (isJobPage(currentUrl, 'Indeed')) {
