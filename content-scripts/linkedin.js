...
import { detectPlatform, isJobPage, extractLinkedInJobData } from '../src/services/detector.js';
...
function init() {
  ...
  if (platform === 'LinkedIn' && isJobPage(url, platform)) {
    detectJob();
    observePageChanges();
  }
}
...
function detectJob() {
  currentJobData = extractLinkedInJobData();
...
function observePageChanges() {
  ...
  const observer = new MutationObserver(() => {
    ...
    if (currentUrl !== lastUrl) {
      lastUrl = currentUrl;
      if (isJobPage(currentUrl, 'LinkedIn')) {
        setTimeout(detectJob, 1000); // Wait for content to load
      } else {
        if (detectorButton) {
          detectorButton.remove();
          detectorButton = null;
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'getCurrentJob') {
    ...
    // Import and use autofill engine
